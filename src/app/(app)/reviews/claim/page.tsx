...
import { getClaimableContactsForClient } from "@/modules/contacts/queries";

export default async function ClaimableContactsPage() {
  const session = await auth.api.getSession({ headers: await headers() });
  if (!session) redirect("/login");

  const contacts = await getClaimableContactsForClient(session.user.id);
    ...
    return (
      <main className="mx-auto max-w-lg px-4 py-8">
        ...
        <p className="mt-4 text-[15px] text-sb-muted">
          No tenés contactos disponibles para iniciar un reclamo.
        </p>
        <Link
          href="/dashboard"
  ...
  return (
    <main className="mx-auto max-w-lg px-4 py-8">
      ...
      <p className="mt-2 text-[15px] text-sb-muted">
        Si un profesional no registró el trabajo que hicieron juntos, podés
        iniciar un reclamo para habilitarte a dejar una reseña.
      </p>
      ...
      <div className="mt-5 flex flex-col gap-3">
        {contacts.map((contact) => (
          <Link
            ...
            href={`/reviews/claim/${contact.contactEventId}`}
